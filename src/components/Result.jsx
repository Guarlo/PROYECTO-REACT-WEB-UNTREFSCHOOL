...
const Result = ({ budgetAmount, formData, setCargado, cargado }) => {
  ...
  const onClickButton = () => {
    ...
    setNameButtonHistory(
      "Agregar a la lista: (Actuales: " + nextId.toString() + ")"
    );
  };
  ...
  const formatedTime1 = (ahora) => {
    // const ahora = new Date(); // Obtener la fecha y hora actual
    //  const horaFormateada = new Intl.DateTimeFormat('es', {
    //   hour: 'numeric',
    //   minute: 'numeric',
    //   second: 'numeric',
    // }).format(ahora);
    ...
    // Formatear la hora en formato hh:mm:ss.SSS (milisegundos)
    ...
    // Obtener los milisegundos y agregarlos al formato
    //const milisegundos = ahora.getMilliseconds();
    //const horaConMilisegundos = `${horaFormateada}.${milisegundos
    //  .toString()
    //  .padStart(3, "0")}`;
    //const myReturn = horaConMilisegundos;
          ...
          {lista.map((item, index) => (
            <li key={item.id}>
              <div className='cardHistorial'>
                <ResumeText><strong className='cotizacion'>Cotización:</strong> {item.id}</ResumeText>
                <hr />
                <ResumeText><strong>Horario:</strong> {formatedTime1(item.dateTime)}</ResumeText>
                <ResumeText><strong>Dispositivo:</strong> {item.device}</ResumeText>
                <ResumeText><strong>Marca:</strong> {item.brand}</ResumeText>
                <ResumeText><strong>Año:</strong> {item.year}</ResumeText>
                <ResumeText><strong>Precio:</strong> {formatedPrice(item.budgetAmount)}</ResumeText>
              </div>
            </li>
          ))}
        </div>
